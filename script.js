(() => {
  ...
  (function initProjectMediaLightbox(){
    ...
    const mediaSelector = 'img.project-page-image, video.project-page-video, img.project-cover-img, video.project-cover-img, iframe';
  ...
  /* ---------------------------------------------------------
     Project-feed video playback
     Videos stay clean and control-free in the case-study page. They
     autoplay muted while visible and pause when scrolled away.
  --------------------------------------------------------- */
  (function initProjectFeedVideos(){
    const selector = '.project-page .project-page-video';
    let observer = null;

    function bind(scope){
      const videos = Array.from((scope || document).querySelectorAll(selector));
    ...
    bind();
    document.addEventListener('ya:projects-rendered', () => bind());
  })();
