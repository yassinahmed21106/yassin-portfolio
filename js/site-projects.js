...
// project-page markup. If Supabase isn't reachable, static HTML stays.
(function () {
  'use strict';
  ...
  function buildProjectPage(project, index) {
    ...
    const images = rawImages.map(entry =>
      typeof entry === 'string' ? { url: entry, type: 'image' } : entry
    );
    if (images.length) {
  ...
  async function render() {
    ...
    try {
      ...
      const branding = bySection.branding || [];
      ...
      // Custom groups created in admin (anything except branding/social).
      Object.keys(bySection).forEach(key => {
        if (key === 'branding' || key === 'social') return;
        const list = bySection[key];
        if (!list.length) return;
        const grid = ensureCategoryGrid(key);
        if (!grid) return;
        grid.innerHTML = '';
        list.forEach((p, i) => grid.appendChild(buildBrandingCard(p, i)));
      });
